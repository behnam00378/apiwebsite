...
exports.editePost = async (req, res, next) => {
  ...
  try {
    ...
    if (!post) {
      const error = new Error("پست وجود ندارد ");
      error.statusCode = 404;
      throw error;
    }
    if (post.user.toString() != req.userId) {
      const error = new Error("شما مجوز ویرایش پست را ندارید ");
      ...
      await post.save();
      res.status(200).json({ message: "تغییرات شما با موفقیت ذخیره شد" });
    }
  } catch (err) {
    next(err);
  }
...
exports.deletePost = async (req, res, next) => {
  try {
    ...
    fs.unlink(filePath, (err) => {
      if (err) {
        const error = new Error("خطایی در حذف پست رخداده است ");
...
exports.createPost = async (req, res, next) => {
  ...
  try {
    ...
    res.status(201).json({ message: "your post created sucseesfully" });
  } catch (err) {
    console.log(err);
    next(err);
  }
  ...
  upload(req, res, async (err) => {
    if (err) {
      if (err.code === "LIMIT_FILE_SIZE") {
        return res
          .status(422)
          .json({ error: "حجم عکس نباید بیشتر از ۴ مگابایت باشد" });
      }
      ...
      if (req.files) {
        ...
        res.status(400).json({ error: "باید عکسی انتخاب کنید " });
      }
    }
  });
};
